  ...
  return (
    <div>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h2>Pengurusan Tapak Projek</h2>
          <p>Setup dan pengurusan tapak pembinaan dengan kawasan geofence</p>
        </div>
        <button className="btn btn-primary">+ Tambah Tapak</button>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: 20 }}>
        {SITES.map((site) => (
          <div key={site.id} className="data-table-container" style={{ padding: 20 }}>
            {/* Map Placeholder */}
            <div style={{
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 6 }}>
                ...
                <span style={{ fontWeight: 700, color: '#f59e0b' }}>{site.radius}m</span>
              </div>
              <input
                type="range"
                min="50"
                max="500"
                defaultValue={site.radius}
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button className="btn btn-outline btn-sm" style={{ flex: 1 }}>📊 Laporan</button>
            </div>
          </div>
        ))}
      </div>
